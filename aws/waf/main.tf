...
resource "aws_iam_role" "lambda_role_reputation_list_parser" {
  ...
  assume_role_policy = element(data.aws_iam_policy_document, count.index)["assume_role.json"] //element(concat(data.aws_iam_policy_document.*.assume_role.json, [""]), 0) // "${data.aws_iam_policy_document.assume_role.json}"
}

...
#          content {
 #           name = lookup(it.value.key, "name", null)
  #          value = lookup(it.value.value, "value", null)
   #       }          
    #    }        
        
       dynamic "custom_header" {
         for_each = length(keys(it.value.custom_header)) > 0 ? [1]: []
         content {
           custom_header = "${it.value.custom_header}"
         }
       }        
